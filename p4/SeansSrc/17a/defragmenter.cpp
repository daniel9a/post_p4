  ...
  drivePtr(diskDrive), currentBlockLoc(2)
{
  ...
  {
    fileSize = drivePtr->directory[contiguous[fileNum].fileNum].getSize();
    fileBlockID = contiguous[fileNum].oldLoc;
    drivePtr->directory[contiguous[fileNum].fileNum].setFirstBlockID(currentBlockLoc);
...
int firstBlockSort(const void *ptr1, const void *ptr2)
{
  return ((FirstBlock*) ptr1)->oldLoc - ((FirstBlock*) ptr2)->oldLoc;
}
...
void Defragmenter::initialize()
{
  ...
  contiguousCount = randomCount = 0;
  
  for(int fileNum = 0; fileNum < drivePtr->getNumFiles(); fileNum++)
  {
    int fileBlockID = drivePtr->directory[fileNum].getFirstBlockID();
    DiskBlock *diskBlockPtr = drivePtr->readDiskBlock(fileBlockID);
    int newLoc = store(diskBlockPtr, fileBlockID);
    
    if(diskBlockPtr->getNext() == fileBlockID + 1)
      contiguous[contiguousCount++] = FirstBlock(fileBlockID, newLoc, fileNum); 
    else
      random[randomCount++] = FirstBlock(fileBlockID, newLoc,fileNum); 
  }

  qsort(contiguous, contiguousCount, sizeof(FirstBlock), firstBlockSort);
  qsort(random, randomCount, sizeof(FirstBlock), firstBlockSort);
  for(int i = contiguousCount, j = 0; j < randomCount; i++, j++)
    contiguous[i] = random[j];
...
int Defragmenter::store(DiskBlock *diskBlockPtr, int blockID)
{
